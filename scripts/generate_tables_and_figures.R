...
cat('- setup parameters\n')
{
    ...
    dataset_label_list <- c('Simulation', 'Real')
...
cat('- stage', stage, 'generate contig summary\n')
{
        ...
        for(species_label in species_label_list){
            ...
            for(assembly_label in assembly_label_list){
                ...
                X <- data.frame(dataset=dataset_label, species=species_label, transcript=prettyNum(nrow(ref), big.mark=num_mark),
                                ...
                                aligned_transcript=prettyNum(length(unique(match$contig_name)), big.mark=num_mark), 
                ...
                X <- data.frame(dataset=dataset_label, species=label, assembly=assembly_label,
                                ...
                                good=round(median(contig$contig_tr_good), 3),
    ...
    figure_list[[5]] <- return_barplot(transrate, 'species', 'good', 'assembly', y_label='Good Mapping', legend_title='Assembly')
    ...
    rm(ref, match, contig, X, contig_best_hit, transcript_best_hit, correct, recover)
        ...
        for(dataset_label in dataset_label_list){
            ...
            for(species_label in species_label_list){
                for(assembly_label in assembly_label_list){
                    ...
                    figure_list[[i]] <- return_cormatrix(cor_matrix, title=title, subtitle=assembly_label, legend_title=title)
                    i <- i + 1
                }
            }
...
cat('- stage', stage, 'generate figure error boxplot and correlation matrix (full-length, incompleteness, over-extension)\n')
{
    ...
    for(dataset_label in dataset_label_list){
        for(species_label in species_label_list){
            cat('    - start analyzing', dataset_label, species_label, '\n')
            figure_list <- list()
            y_label <- paste(dataset_label, species_label)
            row_name[row_index] <- y_label
            i <- 1
            col_index <- 1
            for(assembly_label in assembly_label_list){
                for(j in seq(1:3)){
                    ...
                    xprs_error <- paste0('xprs_error_', xprs_list[j])
                    xprs_label <- xprs_label_list[j]
                    x_label <- paste(assembly_label, xprs_label)
                    
                    X <- fetch_data(match_dataset, dataset_label, species_label, assembly_label)
                    Y <- rbind(categorize_match(X, 'full-length'), categorize_match(X, 'over-extension'), categorize_match(X, 'incompleteness'))
                    
                    pearson[row_index, col_index] <- round(cor(Y[, answer], Y[, xprs]), 3)
                    spearman[row_index, col_index] <- round(cor(Y[, answer], Y[, xprs], method='spearman'), 3)
                    col_name[col_index] <- x_label
    ...
    pearson_figure <- return_cormatrix(pearson, title="Pearson's R", legend_title="Correlation Coefficient")
    spearman_figure <- return_cormatrix(spearman, title="Spearman's R", legend_title="Correlation Coefficient")
        ...
        for(dataset_label in dataset_label_list){
            ...
            for(species_label in species_label_list){
                ...
                for(assembly_label in assembly_label_list){
                    ...
                    match <- categorize_match(X, category)
                    
                    error_df <- data.frame()
                    col_index <- 1
                    for(subcategory in c('max-xprs', 'tot-xprs', 'max-score')){
                        for(i in seq(1:3)){
                            ...
                            if(category == 'family-collapse'){
                                filter_ref <- aggregate_column(ref, 'ref_component_contribute_xprs_answer', 'ref_component', 
                                                                    'ref_component_max_contribute_answer', max)
                            } else if (category == 'duplication'){
                                target_contribute <- paste0('contig_component_contribute_xprs_', xprs_list[i])
                                max_target_contribute <- paste0('contig_component_max_contribute_xprs_', xprs_list[i])
                                filter_contig <- aggregate_column(contig, target_contribute, 'contig_component', max_target_contribute, max)
                            }
                            
                            if (category == 'family-collapse' & subcategory == 'max-xprs'){
                                x_label <- paste(xprs_label, ' (max xprs)')
                                X <- best_hit_filter(X, 'ref_component_contribute_xprs_answer', on='contig_name', func=max)
                            } else if (category == 'family-collapse' & subcategory == 'tot-xprs'){
                                x_label <- paste(xprs_label, ' (tot xprs)')
                                X <- best_hit_filter(X, 'alignment_score', on='contig_name', func=max)
                                answer <- 'ref_component_tot_xprs_answer'
                                xprs_error <- paste0('tot_component_xprs_error_', xprs_list[i])
                                X[, xprs_error] <- (X[, xprs] - X[, answer]) / (X[, xprs] + X[, answer]) * 100 
                                X[is.na(X[, xprs_error]), xprs_error] <- 0 
                            } else if (category == 'family-collapse' & subcategory == 'max-score'){
                                x_label <- paste(xprs_label, ' (max score)')
                                X <- best_hit_filter(X, 'alignment_score', on='contig_name', func=max)
                            } else if (category == 'duplication' & subcategory == 'max-xprs'){
                                x_label <- paste(xprs_label, ' (max xprs)')
                                X <- best_hit_filter(X, paste0('contig_component_contribute_xprs_', xprs_list[i]), on='ref_name', func=max)
                            } else if (category == 'duplication' & subcategory == 'tot-xprs'){
                                x_label <- paste(xprs_label, ' (tot xprs)')
                                X <- best_hit_filter(X, 'alignment_score', on='ref_name', func=max)
                                xprs <- paste0('contig_component_tot_xprs_', xprs_list[i])
                                xprs_error <- paste0('tot_component_xprs_error_', xprs_list[i])
                                X[, xprs_error] <- (X[, xprs] - X[, answer]) / (X[, xprs] + X[, answer]) * 100 
                                X[is.na(X[, xprs_error]), xprs_error] <- 0 
                            } else if (category == 'duplication' & subcategory == 'max-score'){
                                x_label <- paste(xprs_label, ' (max score)')
                                X <- best_hit_filter(X, 'alignment_score', on='ref_name', func=max)
                            }
                            ...
                            error_df <- rbind(error_df, X[, c('x_label', 'error', 'subcategory_label')])
                            pearson[row_index, col_index] <- round(cor(X[, answer], X[, xprs]), 3)
                            spearman[row_index, col_index] <- round(cor(X[, answer], X[, xprs], method='spearman'), 3)
                            col_name[col_index] <- x_label
                            col_index <- col_index + 1
                        }
                    }
                    error_df$x_label = factor(error_df$x_label)
                    error_figure_list[[error_index]] <- return_boxplot(error_df, 'x_label', 'error', 'subcategory_label', cut=F, breaks=seq(-100, 40, by=10), x_label='', y_label='Relative Error (%)', show_guide=F, title=species_label, subtitle=assembly_label)
                    error_index <- error_index + 1
                    row_name[row_index] <- paste(species_label, assembly_label)
                    row_index <- row_index + 1
                }
            }
            rownames(pearson) <- row_name
            ...
            dev.off()
            pearson_figure_list[[cormatrix_index]] <- return_cormatrix(pearson, title=dataset_label, subtitle="Pearson's R", legend_title="Correlation Coefficient")
            spearman_figure_list[[cormatrix_index]] <- return_cormatrix(spearman, title=dataset_label, subtitle="Spearmans's R", legend_title="Correlation Coefficient")
...
cat('- stage', stage, 'generate table', no_table, 'average alignment number for family-collapse and duplication\n')
{
...
cat('- stage', stage, 'generate table', no_table, 'contribute cv for family-collapse and duplication\n')
{
    for(dataset_label in dataset_label_list){
        for(species_label in species_label_list){
            cat('    - start analyzing', dataset_label, species_label, '\n')
            for(assembly_label in assembly_label_list){
                for(category in c('family-collapse', 'duplication')){
                    ...
                    if(category == 'family-collapse'){
                        ref <- subset(ref, ref$ref_component %in% match$ref_component)
                        cv <- ddply(ref, .(ref_component), function(x) mean(x$ref_xprs_answer)/sd(x$ref_xprs_answer))
                        colnames(cv) <- c('component', 'cv')
                        cv[is.na(cv$cv), 'cv'] <- 0
                    } else if (category == 'duplication') {
                        contig <- subset(contig, contig$contig_component %in% match$contig_component)
                        ####FUCK U ALL#####
                        t <- aggregate_column(match, 'contig_component_contribute_xprs_kallisto', 'contig_component', 'max_contribute', fun=max)
                        t <- t[, c('ref_xprs_answer', 'contig_component', 'max_contribute', 'contig_component_max_xprs_kallisto', 'contig_component_tot_xprs_kallisto')]
                        t <- t[!duplicated(t), ]
                        unbalance <- subset(t, t$max_contribute > 0.8)
                        balance <- subset(t, t$max_contribute < 0.5)
                        for(i in seq(1, 3)){
                            xprs <- paste0('contig_xprs_', xprs_list[i])
                            xprs_label <- paste0('CV_', xprs_label_list[i])
                            append_cv <- ddply(contig, .(contig_component), function(x) mean(x[, xprs])/sd(x[, xprs]))
                            colnames(append_cv) <- c('component', xprs_label)
                            append_cv[is.na(append_cv[, xprs_label]), xprs_label] <- 0
                            rownames(append_cv) <- append_cv$component
                            if(i == 1){
                                cv <- append_cv
                            } else {
                                cv[, xprs_label] <- append_cv[cv$component, xprs_label]
                            }
                        }
                    }
                    X <- X[!duplicated(X),]
